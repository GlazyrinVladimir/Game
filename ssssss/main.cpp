...
void playing(RenderWindow &window,Clock &clock,View &view,PLAYER &Player, std::list<Entity*>::iterator &it, std::list<Entity*> &entities, PlayerScores &plScores,float &shootTime_clon,float &shootTime_droid,AnimationManager &anim_bullet, Level &lvl)
{
	while (window.isOpen())
	{
		...
		for (it = entities.begin();it != entities.end();it++)
		{
			...
			definition_of_the_enemy(it, Player, anim_bullet, entities, lvl, time, shootTime_clon, shootTime_droid);
...
void start_game()
{
	...
	playing(window, clock, view, Player, it, entities, plScores, shootTime_clon, shootTime_droid, anim_bullet, lvl);
}

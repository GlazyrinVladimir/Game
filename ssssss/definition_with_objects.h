...
void definition_of_the_enemy(std::list<Entity*>::iterator &it, PLAYER &Player, AnimationManager &anim_bullet, std::list<Entity*> &entities, Level &lvl, float &time, float &shootTime_clon, float &shootTime_droid)
{
	//1. враги
	if ((*it)->Name == "enemy_clon")
	{
		interaction_with_enemy_clon(Player, anim_bullet, entities, it, lvl, time, shootTime_clon);
	}
	if ((*it)->Name == "enemy_droid")
	{
		interaction_with_enemy_droid(Player, anim_bullet, entities, it, lvl, time, shootTime_droid);
	}
}

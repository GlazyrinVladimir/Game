...
public:
	...
	bool life, dir, repulse, hit = false;
...
public:
	...
	void KeyCheck(float time)
	{
		...
		if (key["Up"])
		{
			...
			if (STATE == stay || STATE == walk) { dy = -0.24; STATE = jump; anim.play("jump"); }
		}
...
public:
	...
	void keyCheck_droid(float time)
	{
		if (dx != 0) { anim.set("walk"); }
		else
		{
	...
	{
		std::cout << "new (*it)->x" << hitTime << "\n";
		if (hit) {
			hitTime += time;
			if (hitTime > 60) { hit = false; hitTime = 0; }
			else anim.set("hit");
		}
